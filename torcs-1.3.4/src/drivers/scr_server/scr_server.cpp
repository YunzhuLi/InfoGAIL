...
#define UDP_ID "SCR"
//////// by Yunzhu ////////
#define UDP_DEFAULT_TIMEOUT 1000000
//////// by Yunzhu ////////
...
//////// by Yunzhu ////////
/**** variables for TCP ***/
static int listenSocket_tcp[NBBOTS];
socklen_t clientAddressLength_tcp[NBBOTS];
tSockAddrIn clientAddress_tcp[NBBOTS], serverAddress_tcp[NBBOTS];
static int sessionSocket_tcp[NBBOTS];
//////// by Yunzhu ////////
...
newrace(int index, tCarElt* car, tSituation *s)
{
    ...
    ***********************************************************************************/
    ...
    /***********************************************************************************
    ************************* TCP client identification (by Yunzhu) ********************
    ***********************************************************************************/

    bool identified_tcp = false;
    int iSetOption = 1;

    listenSocket_tcp[index] = socket(AF_INET, SOCK_STREAM, 0);
    if (setsockopt(listenSocket_tcp[index], SOL_SOCKET, SO_REUSEADDR, (char *)&iSetOption,
	       	   sizeof(iSetOption)) < 0)
	    std::cerr << "Error: setsockopt(SO_REUSEADDR) failed";
#ifdef SO_REUSEPORT
    if (setsockopt(listenSocket_tcp[index], SOL_SOCKET, SO_REUSEPORT, (char *)&iSetOption,
		   sizeof(iSetOption)) < 0)
	    std::cerr << "Error: setsockopt(SO_REUSEPORT) failed";
#endif
    if (listenSocket_tcp[index] < 0)
    {
	    std::cerr << "Error: cannot create tcp listenSocket!";
	    exit(1);
    }

    // Bind listen socket to listen port.
    serverAddress_tcp[index].sin_family = AF_INET;
    serverAddress_tcp[index].sin_addr.s_addr = htonl(INADDR_ANY);
    serverAddress_tcp[index].sin_port = htons(TCP_LISTEN_PORT+index);

    if (bind(listenSocket_tcp[index],
             (struct sockaddr *) &serverAddress_tcp[index],
	     sizeof(serverAddress_tcp[index])) < 0)
    {
	    std::cerr << "cannot bind tcp socket";
    	exit(1);
    }

    // Wait for connections from clients.
    listen(listenSocket_tcp[index], 5);
    std::cout << "Waiting for tcp request on port " << TCP_LISTEN_PORT+index << "\n";

    // Loop until a client identifies correctly
    while (!identified_tcp)
    {
	    clientAddressLength_tcp[index] = sizeof(clientAddressLength_tcp[index]);

	    sessionSocket_tcp[index] = accept(listenSocket_tcp[index], 
			    		      (sockaddr *) &clientAddress_tcp[index],
		                              &clientAddressLength_tcp[index]);
	    if (sessionSocket_tcp[index] < 0)
	    {
	        std::cerr << "Error: problem in receiving from the tcp listen socket";
	        exit(1);
	    }

#ifdef __TCP_SERVER_VERBOSE__
	    std::cout << "  tcp connection accepted by " << \
            inet_ntoa(clientAddress_tcp[index].sin_addr);
	    std::cout << ":" << ntohs(clientAddress_tcp[index].sin_port) << "\n";
#endif
...
drive(int index, tCarElt* car, tSituation *s)
{
    ...
    stateString += SimpleParser::stringify("rpm", car->_enginerpm);
    ...
    //////// by Yunzhu ////////
    // Sending the img to the client
    if (car->img != NULL)
    {
        char *pbuf = (char *)(car->img);
        unsigned int bufsize = 320 * 240 * 3;
        char uintStr[20];
        snprintf(uintStr, sizeof(uintStr), "%u;", bufsize);
        if (send(sessionSocket_tcp[index], uintStr, strlen(uintStr), 0) < 0)
            std::cerr << "Error: cannot send file size via tcp";

        do
        {
            unsigned int BUF_SIZE = 1024;
            int num = send(sessionSocket_tcp[index], pbuf, min(BUF_SIZE, bufsize), 0);
            if (num < 0)
                std::cerr << "Error: cannot send image via tcp";
            pbuf += num;
            bufsize -= num;
        }
        while (bufsize > 0);
