...
ReRaceRules(tCarElt *car)
{
	...
	/* Yunzhu - Allow the robot to run forever
	...
	*/
	...
	/* Yunzhu - Do not care if we hit the wall or cut a corner
...
reCapture(void)
{
	...
	for (int i = 0; i < s->_ncars; i++) {
		if (strncmp(s->cars[i]->_name, "scr_server", 10) == 0) {
		// if (strncmp(s->cars[i]->_name, "Player", 6) == 0) {
			car = s->cars[i];
			break;
		}
	}
	
	unsigned char * &img = car->img;
	//////// by Yunzhu ////////
	...
	//////// img = (unsigned char*)malloc(vw * vh * 3);
    	if (img == NULL)
		img = (unsigned char*)calloc(vw * vh * 3, sizeof(*img));
	if (img == NULL) {
		return;
	}
